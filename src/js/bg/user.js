            ...
            .then(({status, data, data: { apitoken, api_token }}) => {
                ...
                if (this.app.api.NOTOK_STATUS.includes(status)) {
                    ...
                    if (errors.username && errors.password) {
                        ...
                        // if the error has something to do with the 2fa token it can be found in the 'two_factor_token' property
                        ...
                        // citchen sink for all other situations
                        this.app.emit('user:login.failed', {reason: status})
                    }
                } else if (this.app.api.OK_STATUS.includes(status)) {
                    if (token) {
                        this.app.store.set('token', `Token ${username}:${token}`)
                        // after the token is stored we can call services since the token is automatically added to the requests
